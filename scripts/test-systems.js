...
console.log("\n== 15d. R15：鎮民一次性支線委託（固定報酬 + 完成紀錄）==");
{
  ...
  for (const npcId of Object.keys(C.NPC_SIDE_QUESTS)) {
    ...
    const req = G.ensureNpcSideQuestRequest(st, npcId, T0 + expectedCoins + 1);
    assert(req && req.sideQuestId === def.id, `${npcId} 生成一次性支線委託`);
    for (const [id, qty] of Object.entries(req.wants)) st.storage.items[id] = (st.storage.items[id] || 0) + qty;
    const before = st.coins;
    const done = G.fulfillNpcRequest(st, npcId, T0 + expectedCoins + 2);
    expectedCoins += def.rewardCoins; expectedXp += def.rewardXp;
    assert(done.ok && done.sideQuestId === def.id && st.coins === before + def.rewardCoins, `${npcId} 支線交付固定小額報酬`);
    assert(G.npcSideQuestStatus(st, npcId).completed === true, `${npcId} 支線完成狀態可查`);
    assert(G.ensureNpcSideQuestRequest(st, npcId, T0 + expectedCoins + 3) === null, `${npcId} 支線完成後不重複生成`);
  }
  ...
  assert(expectedCoins === 52 && expectedXp === 14, `R15 四條支線總增發 ${expectedCoins} 金 / ${expectedXp} XP（固定值可審）`);
}
