...
console.log("\n== 13. 故事任務完成度同步（作物先完成也會補進度）==");
{
  ...
  assert(tutorial.rewardCoins === 45 && tutorial.rewardXp === 3, "新手保底訂單報酬維持 45 金 / 3 XP，不跳過早期升級節奏");
...
console.log("\n== 14d. R64：半壞健康 map 會補齊必要站點、結構、NPC 與橋梁 ==");
{
  const broken = S.defaultState(T0);
  const well = broken.map.tiles.find((t) => t.station === "well");
  const mailbox = broken.map.tiles.find((t) => t.station === "mailbox");
  const orderBoard = broken.map.tiles.find((t) => t.station === "order_board");
  const mayor = broken.map.tiles.find((t) => t.npc === "mayor");
  const coop = broken.map.tiles.find((t) => t.structureId === "coop");
  const bridge = broken.map.tiles.find((t) => t.bridge);
  well.station = null;
  mailbox.station = null;
  orderBoard.station = null;
  mayor.npc = null;
  mayor.npcFacing = null;
  coop.structureId = null;
  coop.blocked = false;
  bridge.bridge = false;
  bridge.terrain = "grass";
  const migrated = S.migrate({
    version: 1,
    coins: 33,
    map: broken.map,
    buildings: broken.buildings,
    animals: broken.animals,
    player: broken.player,
  });
  assert(migrated.map.tiles.find((t) => t.x === 3 && t.y === 10).station === "well", "缺水井站點會補回");
  assert(migrated.map.tiles.find((t) => t.x === 8 && t.y === 5).station === "mailbox", "缺信箱站點會補回");
  assert(migrated.map.tiles.find((t) => t.x === 9 && t.y === 2).station === "order_board", "缺訂單看板站點會補回");
  assert(migrated.map.tiles.some((t) => t.npc === "mayor") && migrated.map.tiles.some((t) => t.npc === "merchant")
    && migrated.map.tiles.some((t) => t.npc === "elder") && migrated.map.tiles.some((t) => t.npc === "child"),
    "缺單一 NPC 時仍補齊所有必要 NPC");
  const coopDef = C.STRUCTURES.find((s) => s.id === "coop");
  assert(migrated.map.tiles.filter((t) => t.structureId === "coop").length === coopDef.w * coopDef.h,
    "缺雞舍 footprint structureId 會補齊");
  const fixedBridge = migrated.map.tiles.find((t) => t.x === 16 && t.y === 4);
  assert(fixedBridge && fixedBridge.bridge === true && fixedBridge.terrain === "water", "缺斷橋旗標/地形會補回");
  assert(migrated.buildings.some((b) => b.structureId === "coop") && migrated.map.tiles.some((t) => t.structureId === "coop" && t.buildingId),
    "預置雞舍 building 記錄與 footprint buildingId 會重新接回");
}
...
console.log("\n== 17. R23：智慧助手 / 離線摘要 / 安全存檔 ==");
{
  ...
  assert(migrated.settings.smartAssistant === false && migrated.settings.offlineSummary === true
    && migrated.settings.soundEnabled === true && migrated.settings.soundVolume === 0.55
    && migrated.settings.performanceMode === "auto" && migrated.settings.textSize === "medium",
    "舊存檔設定遷移保留助手偏好並補離線摘要/音效/效能模式/文字大小預設");
  ...
  const badVolume = S.migrate({ version: 1, coins: 5, settings: { soundVolume: 8 }, map: { width: C.MAP_W, height: C.MAP_H, tiles: [] } });
  assert(badPerf.settings.performanceMode === "auto" && badPerf.settings.textSize === "medium", "舊存檔非法效能/文字設定會回復預設");
  assert(badVolume.settings.soundVolume === 1, "舊存檔音量設定會夾在 0-1 範圍");
