...
7. 若要使用地圖、動物、建築、UI icon 素材，讀 `references/asset-gameplay-integration.md`；素材必須接玩法，不可只放在 repo 或當靜態裝飾。
8. 若要增加可玩性與互動，讀 `references/gameplay-interactions-roadmap.md`，用工具模式、地圖互動、動物產物、混合訂單與角色動畫建立新循環。
9. 若要宣稱「有地圖、可移動、像生活農場 RPG」，必須通過 `references/playable-map-movement-acceptance.md`，不要只做擴張面板或固定角色動畫。
10. 若要把桌機版提升成高品質 RPG 農場，讀 `references/rpg-quality-rework-brief.md`、`references/asset-production-spec-v2.md`、`references/production-directive-rpg-rework.md`，不要用跑版概念圖或 emoji/CSS 色塊當正式地圖。
11. 若人物比例不正常、地圖像固定方塊、或澆水/鋤地/收成沒有 RPG 動作，讀 `references/rpg-action-map-gate.md` 與 `references/sprite-cutting-method-v3.md`，並使用 `art-config-rpg-v3.json` 的角色/動作/VFX/自然地形規格。
12. 若要稽核遊戲本身並交給製作端繼續優化，讀 `references/production-directive-stage4-game-audit.md`，優先處理大地圖、RPG 互動、故事任務、v4 素材 gate，不要只補更多面板。
13. 交給 Claude/Codex 時讀 `references/claude-handoff.md`，保持資料檔、素材檔、驗收步驟一致。
14. 完成後務必實測：種植、收成、訂單、升級、離線重新載入、手機版操作、Console error。
...
- 已生成的地圖、動物、建築、角色與 UI icon 素材必須有玩法用途；不可只作裝飾或閒置檔案。
- 有地圖與可移動角色的版本必須把角色放進地圖層、保存玩家座標，並以瀏覽器測試驗證點地後角色真的位移。
- RPG 品質版本必須使用整數 frame size 或 atlas JSON；不得再用 `1254x1254` 概念圖搭配百分比切片作正式素材。
- RPG 動作版本必須提供角色 walk/action/VFX sheet，澆水、鋤地、播種、收成、收集、站點使用都要有地圖上的可見動畫，不只 toast 或數值變化。
...
- 高品質桌機版應該 world-first：地圖是主角，HUD/訂單/升級是輔助，不讓大卡片壓過場景。
- 高品質地圖不能只顯示固定方塊；需要 terrain edge/corner/overlay、自然路徑、水岸、橋、植物雜訊、陰影和站點。
...
地圖、動物、建築與 icon 如何轉成玩法見 `references/asset-gameplay-integration.md`。
更多可玩性、工具模式、地圖互動、動物循環、事件與測試順序見 `references/gameplay-interactions-roadmap.md`。
